...
@RestController
@RequestMapping("/api/outbound_report")
public class OutboundReportAPI {
    @Autowired
    private OutBoundReportMapper outboundReportMapper;
    @Autowired
    ...
    public Respond<String> addReportA(
        ...
        // 添加报告
        int result = outboundReportMapper.addReport(
                id, username, tel, attachmentStr, goodsStr, nowDate, endDate, note
        );
        if (result == 0) {
    ...
    // 获取出库报告列表
    @GetMapping("/get_report_list")
    public Respond<List<EntityOutboundReport>> getReportList() {
        return new Respond<>(true, "获取成功", outboundReportMapper.getReportsList());
    }
